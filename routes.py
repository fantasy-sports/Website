...
from fancyoptimizer import app, db, bcrypt
from fancyoptimizer.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm
...
posts = [
    ...
    }, 
    {
        ...
        'author': 'Jessica Clifford',
        'title': 'Hot Picks: March 23th',
        'content': 'Second post content',
        'date_posted': '10:01am, 03/04/19'
    }
]
...
@app.route("/")
def opening():
    return render_template('opening.html', title='Fancy Optimizer')
...
def subscribe():
    ...
    if form.validate_on_submit():
        if form.email.data == 'admin@optimizer.com' and form.password.data == 'password': # temporary login credentials - https://youtu.be/UIJKdCIEXUQ (42:15)
            ...
            flash('Subscription Unsuccessful. Please check fields', 'danger')
    return render_template('subscribe.html', title='Subscribe', form=form)

@app.route("/hotPicks")
def hotPicks():
    return render_template('hotPicks.html', title='Hot Picks')

@app.route("/winningLineups")
def winningLineups():
    return render_template('winningLineups.html', title='Winning Lineups')
